...
if (config.scheduler.enableScheduler) {
  // <-- make sure to condition the task to only load if globally enabled!
  initializeTask();
}
```
...
Schedule changes use `PUT /api/release-tracks/:id/virtual/schedule`. The write
replaces `releaseTrackRegistry.snapshot_schedule` atomically and does not clone
the latest snapshot. The next reconciliation pass refreshes or cancels the
track-local cron job and registers any due explicit dates.
