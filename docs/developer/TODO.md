...
- [ ] **Technique conversion should reach revision sync.** Same class as the (fixed) revoke gap: `convertToSubtechnique`/`convertToTechnique` save the new revision via `repository.save` directly — no `::created`/`::updated` fires, so a track pinning the converted object keeps pinning the pre-conversion revision with no capture. Fix candidates: emit the created/updated event from the conversion paths, or subscribe member sync to the `TECHNIQUE_CONVERTED_*` events with a payload adapter (same pattern as `handleStixObjectRevokedEvent`).
