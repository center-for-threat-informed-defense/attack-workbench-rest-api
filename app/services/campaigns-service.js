...
async function addExtraData(campaign, options) {
    await identitiesService.addCreatedByAndModifiedByIdentities(campaign);
}

async function addExtraDataToAll(campaigns, options) {
    await identitiesService.addCreatedByAndModifiedByIdentitiesToAll(campaigns);
}
...
exports.create = async function(data, options) {
    ...
    if (data.stix.type !== 'campaign') {
        throw new Error(attackObjectsService.errors.invalidType);
    }
