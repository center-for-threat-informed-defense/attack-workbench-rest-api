...
class StixBundlesService {
    ...
    attackIdObjectTypes = ['intrusion-set', 'campaign', 'malware', 'tool', 'attack-pattern', 'course-of-action', 'x-mitre-data_source'];
    async requiresAttackId(attackObject) {
        return attackIdObjectTypes.includes(attackObject?.stix.type);
    }

    async hasAttackId(attackObject) {
        if (attackObject) {
    ...
    async removeEmptyArrays(stixObject, propertyNames) {
    ...
    async conformToStixVersion(stixObject, stixVersion) {
        if (stixVersion === '2.0') {
