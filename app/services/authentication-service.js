...
async function getApikeyChallengeFromServer() {
    try {
        const challengeResponse = await request
            .get(`${ config.workbench.restApiBaseUrl }/api/authn/service/apikey-challenge?serviceName=${ config.workbench.authn.apikey.serviceName }`);
...
async function getApikeyAccessTokenFromServer(challengeHash) {
    try {
        const tokenResponse = await request
            .get(`${ config.workbench.restApiBaseUrl }/api/authn/service/apikey-token?serviceName=${ config.workbench.authn.apikey.serviceName }`)
