    ...
    [
      ...
      function (callback1) {
        collectionsService.retrieveById(
          importedCollection.stix.id,
          { versions: 'all' },
          function (err, collections) {
            if (err) {
              return callback1(err);
            } else {
              duplicateCollection = collections.find(
                (collection) =>
                  toEpoch(collection.stix.modified) === toEpoch(importedCollection.stix.modified),
              );
              if (duplicateCollection) {
                if (
                  options.forceImportParameters?.find(
                    (param) => param === forceImportParameters.duplicateCollection,
                  )
                ) {
        ...
        async.eachLimit(
          ...
          function (importObject, callback2a) {
            ...
            if (semver.gt(objectAttackSpecVersion, config.app.attackSpecVersion)) {
              ...
              if (
                options.forceImportParameters.find(
                  (param) => param === forceImportParameters.attackSpecVersionViolations,
                )
              ) {
                ...
                if (err) {
                  ...
                  if (importObject.type === 'marking-definition') {
                    ...
                    if (duplicateObject) {
                      ...
                      importedCollection.workspace.import_categories.duplicates.push(
                        importObject.id,
                      );
                      return callback2a();
                    }
                  ...
                  if (objects.length === 0) {
                    ...
                    if (importObject.revoked && !latestExistingObject.revoked) {
                      ...
                      importedCollection.workspace.import_categories.revocations.push(
                        importObject.id,
                      );
                    } else if (
                      importObject.x_mitre_deprecated &&
                      !latestExistingObject.x_mitre_deprecated
                    ) {
                      // This a newly deprecated object
                      importedCollection.workspace.import_categories.deprecations.push(
                        importObject.id,
                      );
                    } else if (
                      toEpoch(latestExistingObject.stix.modified) < toEpoch(importObject.modified)
                    ) {
                      // TBD: change x_mitre_version comparison from lexical to numerical
                      if (
                        latestExistingObject.stix.x_mitre_version < importObject.x_mitre_version
                      ) {
                        // This a change (same stixId, higher x-mitre-version, later modified)
                        importedCollection.workspace.import_categories.changes.push(
                          importObject.id,
                        );
                      } else if (
                        latestExistingObject.stix.x_mitre_version > importObject.x_mitre_version
                        ...
                        importedCollection.workspace.import_categories.minor_changes.push(
                          importObject.id,
                        );
                      }
                    } else {
                      // Imported object is older than latest existing object
                      importedCollection.workspace.import_categories.out_of_date.push(
                        importObject.id,
                      );
                    }
                  }
                  ...
                  if (
                    importObject.external_references &&
                    ...
                    for (const externalReference of importObject.external_references) {
                      if (
                        ...
                        if (importObject.type === 'intrusion-set') {
                          if (
                            importObject.aliases &&
                            importObject.aliases.includes(externalReference.source_name)
                          ) {
                            isAlias = true;
                          }
                        } else if (
                          importObject.type === 'malware' ||
                          importObject.type === 'tool'
                        ) {
                          if (
                            importObject.x_mitre_aliases &&
                            importObject.x_mitre_aliases.includes(externalReference.source_name)
                          ) {
                            isAlias = true;
                          }
                        }
                  ...
                  if (options.previewOnly) {
                    ...
                    if (importObject.type === 'marking-definition') {
                      ...
                      service
                        ...
                        .catch(function (err) {
                          if (
                            err.message === service.errors?.duplicateId ||
            ...
            async.eachLimit(
              ...
              function (importReference, callback3a) {
                if (existingReferences.has(importReference.source_name)) {
                  ...
                  if (options.previewOnly) {
                    // Do nothing
                    importedCollection.workspace.import_references.additions.push(
                      importReference.source_name,
                    );
                    return callback3a();
                  } else {
      ...
      function (callback4) {
        if (duplicateCollection) {
          ...
          if (options.previewOnly) {
            ...
            collectionsService
              .create(importedCollection, options)
    ...
    if (bundleObject.type === 'attack-pattern') {
      ...
      if (enterpriseDomain && !icsDomain) {
        ...
        if (Array.isArray(bundleObject.x_mitre_data_sources)) {
          bundleObject.x_mitre_data_sources = bundleObject.x_mitre_data_sources.filter((source) =>
