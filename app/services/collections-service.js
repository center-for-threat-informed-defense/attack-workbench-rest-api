...
exports.retrieveAll = function (options, callback) {
  ...
  Collection.aggregate(aggregation, function (err, collections) {
    if (err) {
      ...
      identitiesService.addCreatedByAndModifiedByIdentitiesToAll(collections).then(function () {
          ...
          if (options.retrieveContents) {
            getContents(collection.stix.x_mitre_contents, function (err, contents) {
              if (err) {
                ...
                collection.contents = contents;
                identitiesService
                  .addCreatedByAndModifiedByIdentities(collection)
...
exports.create = async function (data, options) {
  ...
  if (!options.import) {
    ...
    collection.stix.x_mitre_attack_spec_version =
  ...
  try {
    ...
    if (options.addObjectsToCollection) {
      insertionErrors = await addObjectsToCollection(
