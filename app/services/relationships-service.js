...
class RelationshipsService extends BaseService {
    ...
    if (options.sourceType || options.targetType) {
      ...
      if (options.sourceType) {
        filteredDocs = filteredDocs.filter((document) => {
          ...
          return (
            RelationshipsService.objectTypeMap.get(document.source_objects[0].stix.type) ===
            options.sourceType
          );
        });
      }
      ...
      if (options.targetType) {
        filteredDocs = filteredDocs.filter((document) => {
          ...
          return (
            RelationshipsService.objectTypeMap.get(document.target_objects[0].stix.type) ===
            options.targetType
          );
        });
      }
  ...
  async _enrichMetadata(data, options) {
    ...
    const organizationIdentityRef =
      await this.systemConfigService.retrieveOrganizationIdentityRef();
