...
const availableMechanisms = new Map([
  ['oidc', oidcConfig],
...
exports.configurePassport = async function (mechanismName) {
  ...
  if (mechanism) {
    try {
      ...
      logger.error(
        `Unable to configure system with authentication mechanism ${mechanismName}`,
        err,
      );
    }
  } else {
    logger.error(
      `Unable to configure system with unknown authentication mechanism: ${mechanismName}`,
    );
    throw new Error(
      `Unable to configure system with unknown authentication mechanism: ${mechanismName}`,
    );
...
exports.isServiceAuthenticationMechanismEnabled = function (routeMechanism) {
  return function (req, res, next) {
    if (routeMechanism === 'challenge-apikey' && config.serviceAuthn.challengeApikey.enable) {
      ...
      routeMechanism === 'client-credentials' &&
