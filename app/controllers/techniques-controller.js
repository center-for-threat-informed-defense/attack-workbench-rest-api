...
exports.retrieveVersionById = async function(req, res) {
        ...
        try {
            const technique =  await techniquesService.retrieveVersionById(req.params.stixId, req.params.modified);
            if (!technique) {
                return res.status(404).send('Technique not found.');
            }
            else {
...
exports.updateFull = async function(req, res) {
    ...
    try {
            // Create the technique
        const technique = await techniquesService.updateFull(req.params.stixId, req.params.modified, techniqueData);
        if (!technique) {
            return res.status(404).send('Technique not found.');
        } else {
...
exports.deleteVersionById = async function(req, res) {
    ...
    try {
        const technique = await techniquesService.deleteVersionById(req.params.stixId, req.params.modified);
        if (!technique) {
            return res.status(404).send('Technique not found.');
        } else {
