...
exports.apikeyGetChallenge = function(req, res) {
    try {
        ...
        const challenge = authnService.createChallenge(serviceName);
        logger.debug('Success: Service account challenge created.');
        return res.status(200).send({ challenge });
    }
    catch(err) {
        if (err.message === authnService.errors.serviceNotFound) {
...
exports.apikeyGetToken = function(req, res) {
    try {
        ...
        const token = authnService.createToken(serviceName, challengeHash);
        ...
        if (err.message === authnService.errors.invalidChallengeHash) {
        ...
        else if (err.message === authnService.errors.challengeNotFound) {
