...
exports.retrieveVersionById = async function(req, res) {
        ...
        try {
            const campaign = await campaignsService.retrieveVersionById(req.params.stixId, req.params.modified);
            if (!campaign) {
                return res.status(404).send('Campaign not found.');
            }
            else {
...
exports.updateFull = async function(req, res) {
    ...
    const campaignData = req.body;
    try {
        const campaign = await campaignsService.updateFull(req.params.stixId, req.params.modified, campaignData);
        if (!campaign) {
            return res.status(404).send('Campaign not found.');
        } else {
...
exports.deleteVersionById = async function(req, res) {
    ...
    try {
        const campaign = await campaignsService.deleteVersionById(req.params.stixId, req.params.modified);
        if (!campaign) {
            return res.status(404).send('Campaign not found.');
        } else {
