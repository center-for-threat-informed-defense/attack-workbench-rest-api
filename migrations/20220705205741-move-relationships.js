...
module.exports = {
  async up(db, client) {
    ...
    const oldRelationships = await attackObjectsCollection
      .find({ __t: 'RelationshipModel' })
      .toArray();
    ...
    for (const relationship of oldRelationships) {
      ...
      const newRelationship = await relationshipsCollection.findOne({
        'stix.id': relationship.stix.id,
        'stix.modified': relationship.stix.modified,
      });
      if (newRelationship) {
        console.log(
          'Relationship already exists in relationships collection. Will delete from attackObjects collection without adding to relationships collection.',
        );
        attackObjectsCollection.findOneAndDelete({
          'stix.id': relationship.stix.id,
          'stix.modified': relationship.stix.modified,
        });
      } else {
  ...
  async down(db, client) {
    ...
    for (const relationship of newRelationships) {
      ...
      const oldRelationship = await attackObjectsCollection.findOne({
        'stix.id': relationship.stix.id,
        'stix.modified': relationship.stix.modified,
      });
      if (oldRelationship) {
        console.log(
          'Relationship already exists in attackObjects collection. Will delete from attackObjects collection without adding to attackObjects collection.',
        );
        relationshipsCollection.findOneAndDelete({
          'stix.id': relationship.stix.id,
          'stix.modified': relationship.stix.modified,
        });
      } else {
